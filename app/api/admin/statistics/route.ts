...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!profile || (!['admin', 'coach', 'asst_coach', 'data_admin', 'club_captain'].includes(profile.role) && !hasClubCaptainAccess)) {
      return NextResponse.json(
    ...
    const [
      ...
      supabaseAdmin.from('user_profiles').select('*', { count: 'exact', head: true }).neq('role', 'player'),
